...
- ✅ **Batch 13 — Today view consolidation (A1)** + V2 cleanup (done 2026-06-13; user chose to keep V1) — Removed the experimental Mail-style `TodayViewV2`: deleted the file, `AppRouter` now renders `TodayView()` directly, and dropped `AppearanceManager.TodayViewVersion`/`todayViewVersion`/its key + the "Today View" picker section in `AppearanceSettingsView`. Also removed the redundant **`v2` add-transaction entry layout** (`TransactionEntryLayout` is now just `v1`/`v3`; `current(from:)` maps any stale `"v2"` → `v1`). Plus a new **V3 sub-option** (user request): `InlineComposerPrefs.saveClosesKey` toggle ("Close After Saving") in Settings → Input Method — when off, the composer stays open after sending for rapid multi-entry (gates `onDismiss` in the composer's `send()`). Build + 56 tests green.
- ✅ **Phase 8.5 — Liquid Glass composer** (done 2026-06-13; verified on simulator) — Reworked the V3 inline composer from a small sheet into a **Liquid Glass** keyboard-riding bar (iOS 26 `GlassEffectContainer`/`.glassEffect`), pixel-matching the Messages composer / search bar. Field, category bubble, and send button are glass capsules/circles; the **parsed result surfaces as its own glass bubble above the field**; the send button is a blue glass circle when armed. Presented via `.inlineComposer(vm:)` as a `.safeAreaInset(.bottom)` (not a sheet); the Filter/Search/Add bottom bar is hidden while it's up (`.toolbarVisibility`) so the composer is the only bottom element. Dismiss via a grabber handle (tap/swipe). Verified on the simulator: glass rendering, "100k grabfood yesterday" → glass summary "Rp 100,000 · Yesterday · 🛵 Food Delivery" + blue send, entry created under Yesterday, grabber dismiss restores the toolbar. 56 tests green. CONTEXT updated.
- ✅ **Phase 8.5 — Inline Entry (Experimental V3)** (done 2026-06-13; verified on simulator) — Optional Messages-style inline composer for natural-language entry, gated behind a Settings → Input Method picker (`TransactionEntryLayout` v1/v2/v3; v1/v2 = classic sheet, v3 = inline). New `InlineTransactionParser` (reuses `TransactionParser` for amounts + `CategoryPredictor` for category, adds relative-date parsing: today/yesterday/tomorrow/"N days ago/from now"), `InlineTransactionEntryViewModel` (debounced parse, manual category override, `canSend` = amount+category), and `InlineTransactionEntryView` (category-bubble menu · NL field · send button), presented as a short keyboard-riding sheet from `TodaySheetsModifier`; new entries animate into the list via the existing `.ankaDataDidChange` pipeline. New `InlineTransactionParserTests` (9). Verified end-to-end: "100k grabfood yesterday" → Rp 100,000 / Food Delivery / grabfood under a Yesterday section; amount-only keeps send disabled; bubble override enables it. 56 tests across 7 suites green. CONTEXT updated (Phase 8.5 section).
- ✅ **App Lock — Face ID default on every open** (done 2026-06-13; verified on simulator) — Face ID / Touch ID is now the default unlock method on **every** foreground, not just cold launch. Driven by `@Environment(\.scenePhase)` in `AppLockView` (`.task` for already-active appearances + `onChange(scenePhase → .active)` for foreground re-entry); the prompt is **not** fired while the lock cover appears during backgrounding (that was the bug — the suppressed prompt failed and dumped the user to PIN on reopen). New `AppLockManager.BiometricOutcome` (`.success`/`.failed`/`.unavailable`) lets the view distinguish a real failure (→ PIN fallback) from "couldn't present the prompt" (→ stay on Face ID); the auto-attempt settles ~300ms and retries once at ~600ms to ride out the foreground handoff. A fresh foreground also clears a stale PIN fallback and re-offers Face ID without interrupting a PIN already being typed. Verified end-to-end (Face ID enrolled): cold-launch auto-prompt, **reopen-from-background auto-prompt** (the fix), matching-face unlock, and PIN fallback intact. 47 tests green; CONTEXT App Lock section updated.
