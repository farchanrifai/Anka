...
**Zoom from the `+`** — the bar's transition is `.scale(scale: 0.2, anchor: .bottomTrailing).combined(with: .opacity)`, so it grows out of / shrinks back into the bottom-trailing corner (where the `+` toolbar button sits) — echoing V1's `.navigationTransition(.zoom)` sheet, which can't be used here because that API is sheet/navigation-only, not for a `safeAreaInset`. The scale anchor is a corner (no vertical *move*), so it composes with the keyboard ride rather than fighting it.
