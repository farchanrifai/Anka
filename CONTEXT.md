...
**Focus race (rapid close→reopen)** — focusing immediately on appear is what makes the keyboard rise *with* the bar, but a quick close→reopen can swallow that focus (the previous keyboard is still dismissing, and/or the bar's removal animation gets interrupted and the *same view instance is reused* so the one-shot `.task` never re-runs) → bar shows, no keyboard. Two-part guard: the host increments `openID` on every open and the view keys its focus task `.task(id: openID)` (re-runs even on a reused instance); and after focusing it waits ~250ms and, if `keyboardDidShowNotification` hasn't fired (`keyboardUp`), re-asserts focus (`false`→`true`). Don't collapse this back to a plain `.task { focused = true }`.
