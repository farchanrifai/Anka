...
A Messages-style inline composer for fast natural-language entry, gated behind a Settings picker. **Experimental / optional** — does not replace the classic Add sheet.

**Entry-layout setting** — `TransactionEntryLayout` enum (`v1`/`v3`, `@AppStorage(TransactionEntryLayout.storageKey)`, default `v1`). `v1` presents the classic `AddTransactionView` sheet; `v3` opens the inline composer. Picker lives in Settings → **Input Method**. (A redundant `v2` sheet variant was removed; `TransactionEntryLayout.current(from:)` maps any stale `"v2"` to `v1`.) **V3 sub-option** — `InlineComposerPrefs.saveClosesKey` (`@AppStorage`, default true): whether sending closes the composer or keeps it open for rapid multi-entry (gates the `onDismiss` in the composer's `send()`).

**Wiring** — `AddToolbarButton` (the `+`) reads the layout: `v3` → `vm.showInlineComposer.toggle()`, else `vm.showAddTransaction = true`. The composer is **not a sheet** — it's applied via `.inlineComposer(vm:)` (`InlineComposerModifier`) as a `.safeAreaInset(edge: .bottom)` on `TodayView`, so it rides the keyboard like the Messages composer / system search bar. While it's up, the Filter/Search/Add bottom bar is hidden (`.toolbarVisibility(… , for: .bottomBar)`) so the glass composer is the only bottom element and the coral `+` doesn't bleed behind it. **Dismiss:** there's no grabber — **saving** closes it (back to the default bottom bar), and a near-invisible tap-catcher `.overlay` over the list (applied *before* the inset, so it covers the content but not the composer) dismisses on tap-outside.

**Liquid Glass styling (iOS 26)** — `InlineTransactionEntryView` renders in `GlassEffectContainer` with `.glassEffect(.regular …, in: .capsule/.circle)` on the field, category bubble, send button, and the parsed-result bubble (each with a `glassEffectID` in a shared `@Namespace` so they blend/morph). The **parsed result surfaces as its own centered glass capsule above the field**, laid out **`Date · Note · Rp amount`**; on save it shrinks + flies up toward the list (asymmetric transition) — an approximate "expand into the list" effect synchronized with the new row's insertion (a `Task.sleep(240ms)` defers the close so the fly-up reads first). The send button is `.glassEffect(.regular.tint(DSColor.accent).interactive())` (the app's coral accent) when armed, plain `.regular` glass + gray arrow when disabled.

**Asymmetric transition** — the bar's transition is `.asymmetric(insertion: .opacity, removal: .scale(scale: 0.2, anchor: .bottomTrailing).combined(with: .opacity))`. Open is plain opacity (the bar just fades in while the keyboard lifts the safe-area inset — no zoom). Close **shrinks into the bottom-trailing corner** where the `+` sits. (A symmetric corner-scale zoom-from-`+` on open was tried and reverted — only the close zooms now.)

**Asymmetric open/close timing** — open and close are animated **explicitly** (no single implicit `.animation(value:)`), so they can differ: `AddToolbarButton` opens with `withAnimation(.dsSnappy)`; `dismiss()` closes with `withAnimation(.smooth(duration: 0.4))` (slower, search-bar pace) *and* resigns first responder in the same call, so the shrink and the keyboard descent start together — no 250ms wait for the keyboard to land first.

**Focus lives in the host, not the bar** — a rapid close→reopen used to leave the bar showing with no keyboard: the bar's `@FocusState` got torn down/reused so its one-shot focus never re-fired. (An earlier "re-assert focus after 250ms if `keyboardDidShow` hasn't fired" recovery *did* bring the keyboard up but flickered open→close, because that notification is unreliable mid-transition.) Fix: `InlineComposerModifier` owns `@FocusState composerFocused` (stable across the bar's lifetime) and passes it down as a `FocusState<Bool>.Binding` (`.focused(focus)`). On open, `onChange(showInlineComposer)` defers one runloop (`Task { @MainActor in composerFocused = true }`) so the field is mounted, then focuses; `dismiss()` sets it `false`. No re-assert toggling — the keyboard just follows the field's presence + this one focus set. Don't move the `@FocusState` back into the bar view.

**Keyboard sync** — the bar must rise/fall *with* the keyboard as one motion (not stagger). Two things make this work: the composer's `.task` focuses the field **immediately** (no delay), so the keyboard starts rising the same beat the bar appears; and the bar's transition is **`.opacity` only** (no `.move`), so its sole vertical motion is the keyboard lifting the safe-area inset. Don't reintroduce a focus delay or a slide transition — that's exactly what caused the "bar moves first, keyboard follows" stagger. **Closing is symmetric:** `InlineComposerModifier.dismiss()` resigns first responder (`UIApplication.sendAction(resignFirstResponder)`) so the keyboard descends and the inset bar rides **down with it**, then removes the bar after ~250ms — removing it immediately left it fading in place up top (the close stagger). All exits route through this one `dismiss` (the tap-outside catcher, the `onDismiss` passed to the view, and save after its fly-up). The **return key** closes too: `onSubmit` sends when armed, otherwise calls `onDismiss` (so a stray return never leaves an empty bar behind a dismissed keyboard).
...
**View** — `InlineTransactionEntryView`: category bubble (a `Menu` of expense categories, shows the effective category's emoji) · NL text field (auto-focused, debounced `onChange`, `onSubmit` sends) · accent glass send button (success haptic). The parsed result shows above the field as the centered glass summary capsule (see Liquid Glass styling above). Saving closes the composer.
