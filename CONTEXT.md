...
**Wiring** — `AddToolbarButton` (the `+`) reads the layout: `v3` → `vm.showInlineComposer.toggle()`, else `vm.showAddTransaction = true`. The composer is **not a sheet** — it's applied via `.inlineComposer(vm:)` (`InlineComposerModifier`) as a `.safeAreaInset(edge: .bottom)` on both Today views, so it rides the keyboard like the Messages composer / system search bar. While it's up, the Filter/Search/Add bottom bar is hidden (`.toolbarVisibility(… , for: .bottomBar)`) so the glass composer is the only bottom element and the coral `+` doesn't bleed behind it. **Dismiss:** there's no grabber — **saving** closes it (back to the default bottom bar), and a near-invisible tap-catcher `.overlay` over the list (applied *before* the inset, so it covers the content but not the composer) dismisses on tap-outside.
