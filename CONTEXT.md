...
**Wiring** — `AddToolbarButton` (the `+`) reads the layout: `v3` → `vm.showInlineComposer.toggle()`, else `vm.showAddTransaction = true`. The composer is **not a sheet** — it's applied via `.inlineComposer(vm:)` (`InlineComposerModifier`) as a `.safeAreaInset(edge: .bottom)` on both Today views, so it rides the keyboard like the Messages composer / system search bar. While it's up, the Filter/Search/Add bottom bar is hidden (`.toolbarVisibility(… , for: .bottomBar)`) so the glass composer is the only bottom element and the coral `+` doesn't bleed behind it. Dismiss via the **grabber handle** (tap or swipe-down) at the top of the composer — the gesture lives on the grabber, off the text field, so it doesn't fight the field's selection gestures.

**Liquid Glass styling (iOS 26)** — `InlineTransactionEntryView` renders in `GlassEffectContainer` with `.glassEffect(.regular …, in: .capsule/.circle)` on the field, category bubble, send button, and the parsed-result bubble (each with a `glassEffectID` in a shared `@Namespace` so they blend/morph). The **parsed result surfaces as its own glass capsule above the field** ("Rp amount · Date · Category"). The send button is a `.glassEffect(.regular.tint(sendBlue).interactive())` blue circle when armed (Messages send), plain `.regular` glass + gray arrow when disabled. `sendBlue` is the one hardcoded color (no DS blue token; matches the system composer).
...
**View** — `InlineTransactionEntryView`: grabber handle (tap/swipe to dismiss) · category bubble (a `Menu` of expense categories, shows the effective category's emoji) · NL text field (auto-focused, debounced `onChange`, `onSubmit` sends) · blue glass send button (success haptic). The parsed result shows above the field as the glass summary capsule (see Liquid Glass styling above).
